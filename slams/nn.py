...
class ParamDecoder(nn.Module):
    def __init__(
        ...
        if spatial == 1:
            self.cnn_layers = nn.Sequential(
                nn.Conv1d(in_features, hidden_features, kernel_size=5),
                nn.SiLU(),
                nn.MaxPool1d(kernel_size=5)
            )
            
            self.lstm = nn.LSTM(input_size=hidden_features, hidden_size=hidden_features * 2, batch_first=True)
            self.fc = nn.Linear(hidden_features * 2, out_features)
            
            
        else:
            self.cnn_layers = nn.Sequential(
                nn.Conv2d(in_features, hidden_features, kernel_size=5),
                nn.BatchNorm2d(hidden_features),
                nn.SiLU(),
                nn.Dropout(),
                nn.MaxPool2d(kernel_size=5),
                
                nn.Conv2d(hidden_features, hidden_features, kernel_size=5),
                nn.BatchNorm2d(hidden_features),
                nn.SiLU(),
                nn.Dropout(),
                nn.MaxPool2d(kernel_size=5)
    ...
    def forward(self, x):
        ...
        if self.spatial == 1:
            ...
            B, L, C, H, W = x.shape
            x = self.cnn_layers(x.view(B*L, C, H, W))
            x, (hn, cn) = self.lstm(x.view(B, L, self.hidden_features))
            x = self.fc(self.dropout(hn[-1, :, :]))
