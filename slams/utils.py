...
from slams import config, model_da
...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> Tuple[Tensor, Dict]:
        ...
        if isinstance(self.data[i], (str, Path)):
            x = torch.load(self.data[i])
        else:
            x = self.data[i]
...
class ERA5Dataset(Dataset):
    def __init__(
        ...
        self.window = window
        self.flatten = flatten
        self.data_dir = Path(config.ERA_DATADIR) / 'era5'
        self.normalization_file = Path(config.ERA_DATADIR) / 'climatology' / 'climatology_era5.zarr'
        ...
        self.normalization = self.normalization.sel(param=[f't-{pressure_l}' for pressure_l in config.PRESSURE_LEVELS])
...
def get_latent(latent, x):
    ...
    with torch.no_grad():
        for n in range(n_samples):
            z = latent.encoder(x[n].float().to(device))
            z = z.detach().cpu()
            all_z.append(z)
...
def load_model_from_checkpoint(model_name, version_num, from_root=True):
    log_dir = Path('logs') / model_name if from_root else Path('../logs') / model_name
    config_filepath = Path('slams/configs') / f'{model_name}.yaml' if from_root else Path('../slams/configs') / f'{model_name}.yaml'
    ...
    # Initialize model
    baseline = model_da.LSDA(model_args=model_args, data_args=data_args).to(device)
...
def plot_assimilation_results(true, coarse, assimilated, param_idx, n_step=3, is_sparse=False, save_file=None):
    """Helper function to plot assimilation results
    """
    cmap = 'RdBu_r' if is_sparse else sns.cm.icefire
    
    # Plot true
    f, ax = plt.subplots(4, n_step, figsize=(8,6))
    plt.subplots_adjust(wspace=0.02, hspace=0.02)
    
    for N in range(n_step):
        im = ax[0,N].imshow(true[N,param_idx], cmap=sns.cm.icefire, vmin=-2, vmax=2)
        ax[0,N].axis('off')

    # Plot coarsen
    for N in range(n_step):
        ax[1,N].imshow(coarse[N,param_idx], cmap=cmap, vmin=-2, vmax=2)
        ax[1,N].axis('off')

    # Plot assimilation + residual
    for M in range(assimilated.shape[0]):
        ...
        for N in range(n_step):
            ...
            ax[3,N].imshow(x_sample[N,param_idx] - true[N, param_idx], cmap=sns.cm.icefire, vmin=-2, vmax=2)
            ax[3,N].axis('off')
...
def plot_and_compute_distributions(
    true, assimilated, 
    param_idx, 
    save_file=None
):
...
def sort_numerically(paths):
    "Given list of Path, sort based on number indices instead of lexicography"
    ...
    return sorted(paths, key=_extract_numbers)